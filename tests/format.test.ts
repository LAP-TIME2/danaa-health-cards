...
describe("formatCard", () => {
  ...
  it("renders an auto-checkin continuation marker", () => {
    const rendered = formatAutoCardPrompt({
      has_question: true,
      lease_id: "lease-2",
      bundle_key: "bundle_1",
      bundle_name: "Sleep",
    ...
    expect(rendered).toContain("DANAA_CARD_PENDING");
    expect(rendered).toContain("번호 / skip / 30분 뒤 / 오늘 그만");
  });
});
